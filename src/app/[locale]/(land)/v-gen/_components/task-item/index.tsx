...
const TaskItem = ({ top, taskData }: TaskItemProps) => {
  ...
  const {
    ...
    retry: 0,
    enabled: !taskData.result && !taskData.error,
  });
  ...
  useEffect(() => {
    if (videoData) {
      if (videoData.videoUrls) {
        ...
        videoData.videoUrls.forEach((url: string) => {
          addHistory(
            { ...taskData, result: { videoUrl: url } },
  ...
  }, [createVideoError, taskId, updateTaskResult]);
        ...
        <div className="relative w-full">
          ...
          {createVideoError ? (
            ...
            <div className="min-h-40 w-full">
              {taskData.result ? (
                Array.isArray(taskData.result.videoUrls) ? (
                  <div className="flex flex-col gap-2">
                    {taskData.result.videoUrls.map((url: string) => (
                ...
                <div className="flex min-h-[280px] w-full flex-col items-center justify-center rounded-md bg-slate-50 dark:bg-slate-900" />
              )}
            </div>
          )}
        </div>
            ...
            {extensionType === "style" && (
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value) =>
                    setSelectedSeconds(Number.parseInt(value))
                  }
                >
